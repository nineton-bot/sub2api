<template>
  <BaseDialog
    :show="show"
    :title="t('admin.invoices.dialogs.reject.title')"
    width="narrow"
    @close="onClose"
  >
    <div class="space-y-3">
      <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
        {{ t('admin.invoices.dialogs.reject.reasonLabel') }}
      </label>
      <textarea
        v-model="reason"
        rows="4"
        class="input w-full"
        :placeholder="t('admin.invoices.dialogs.reject.reasonRequired')"
      />
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        <button
          ...
          @click="submit"
        >
          {{ submitting ? t('common.processing') : t('admin.invoices.dialogs.reject.confirm') }}
        </button>
      </div>
    </template>
  </BaseDialog>
...
async function submit() {
  ...
  errorMsg.value = ''
  try {
    await adminInvoiceAPI.reject(props.invoiceId, reason.value.trim())
    emit('submitted')
    onClose()
  } catch (err) {
