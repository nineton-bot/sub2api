...
目标：
- 在账号管理中，为 `apikey` / `bedrock` 类型账号的“配置限额”增加新的计量方式：`金额` / `请求次数`。
- 兼容现有账号级配额的三个维度：`日` / `周` / `总`。

要求：
- 保持现有 `quota_limit` / `quota_daily_limit` / `quota_weekly_limit` 与 `quota_used` / `quota_daily_used` / `quota_weekly_used` 结构不变。
- 新增 `quota_meter` 字段，默认值为 `cost`，请求次数模式为 `requests`。
- 金额模式继续按现有成本累加；请求次数模式改为每次成功请求 `+1`。
- 保持现有 `rolling` / `fixed` 重置方式兼容。
- API Key / Bedrock 账号的调度可用性判断应同时支持金额模式与请求次数模式。

### TODO 2：账号级请求次数配额的统计与展示完善

目标：
- 账号管理中，账号容量徽章、配额进度条、窗口统计等展示，需要兼容 `quota_meter=requests`。

要求：
- `QuotaLimitCard` 支持选择限额类型：`金额` / `请求次数`。
- `AccountCapacityCell`、`AccountUsageCell`、相关账号详情弹窗在请求次数模式下：
  - 不再显示美元符号；
  - 显示请求次数上限与已用次数；
  - 日/周窗口统计按请求次数解释；
  - 总额度按累计请求次数解释。
- 账号编辑/创建时，前端和后端都要正确透传 `quota_meter`。
- 需要补充联调验证：请求次数型账号配额达到限制后，账号应暂停调度，并且页面统计同步更新。
